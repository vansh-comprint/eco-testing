...
def get_scoped_filters(current_user: User) -> Dict[str, Any]:
    ...
    Scoping rules:
    - Super Admin / OPS Admin: No filter (access to all)
    ...
    # Note: For branch-level scoping of assets/batches/employees,
    # callers should use get_it_admin_branch_ids() to get all managed
    # branch IDs. The single branch_id here is a fallback.
...
async def get_it_admin_branch_ids(db, user_id: str) -> List[str]:
    """
    Get all branch IDs managed by an IT Admin.

    Queries the branches table for all branches where it_admin_id matches.
    Used for multi-branch scoping of assets, batches, and employees.
    """
    from sqlalchemy import select
    from app.models.enterprise import Branch

    result = await db.execute(
        select(Branch.id).where(Branch.it_admin_id == user_id)
    )
