...
export function EnterpriseList() {
  ...
  const filteredEnterprises = enterprises
    .filter(e =>
      ...
      e.gstNumber?.toLowerCase().includes(searchQuery.toLowerCase())
    )
    ...
    <div className="space-y-6">
          ...
          {filteredEnterprises.map((enterprise, idx) => {
            ...
            return (
              <motion.div
                ...
                className="border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] hover:border-white/20 transition-all"
              >
                <div className="p-5 border-b border-slate-200 dark:border-white/10">
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 border border-slate-200 dark:border-white/10 bg-white/5 flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {enterprise.gstNumber && (
                        <p className="font-mono text-xs text-slate-500 dark:text-white/50 mt-1">GST: {enterprise.gstNumber}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="space-y-2">
                    {enterprise.contactPerson && (
                      <p className="font-display text-sm text-white">{enterprise.contactPerson}</p>
                    )}
                    {enterprise.contactEmail && (
                      <div className="flex items-center gap-2 text-slate-500 dark:text-white/50">
                        <Mail className="w-4 h-4" />
                        <span className="font-mono text-xs">{enterprise.contactEmail}</span>
                      </div>
                    )}
                    {enterprise.contactPhone && (
                      <div className="flex items-center gap-2 text-slate-500 dark:text-white/50">
                        <Phone className="w-4 h-4" />
                        <span className="font-mono text-xs">{enterprise.contactPhone}</span>
                      </div>
                    )}
                    {enterprise.address && (
                      <div className="flex items-center gap-2 text-slate-500 dark:text-white/50">
                  ...
                  <div className="grid grid-cols-4 gap-3 pt-4 border-t border-slate-200 dark:border-white/10">
                    <div>
                      <p className="font-brand font-bold text-xl text-white">{stats.assetCount}</p>
                      <p className="font-mono text-[10px] text-slate-500 dark:text-white/50 uppercase">Assets</p>
                    </div>
                    <div>
                      <p className="font-brand font-bold text-xl text-white">{stats.batchCount}</p>
                ...
                <div className="p-4 border-t border-slate-200 dark:border-white/10">
                  <button
                    ...
                    className="w-full interactive py-2.5 border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] text-white font-mono font-bold text-xs uppercase tracking-widest hover:bg-white/[0.05] transition-all flex items-center justify-center gap-2"
                  >
                    View Details
      ...
      >
        <div className="border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] p-5">
          ...
          <p className="font-brand font-bold text-3xl text-white">{enterprises.length}</p>
        </div>
        <div className="border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] p-5">
