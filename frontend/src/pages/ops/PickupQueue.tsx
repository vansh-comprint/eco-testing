...
import { useAuth, useAllPickupRequests, useLogisticsAdmins, useCreateLogisticsAdmin, useAssignToLogisticsAdmin } from '@/hooks';
...
export function PickupQueue() {
  ...
  const [showReassignMode, setShowReassignMode] = useState(false);  // V3.2: Reassign mode

  // Get active logistics admins
  const activeLogisticsAdmins = logisticsAdmins.filter(a => a.status === 'active');

  // Filter pickup requests (respects global enterprise filter)
  ...
  const handleAssign = async () => {
    ...
    setShowReassignMode(false);  // V3.2: Reset reassign mode
  };

  const handleCreateAdmin = async () => {
    if (!newAdminName || !newAdminCompany || !newAdminEmail || !newAdminPassword) {
      alert('Name, company, email, and password are required');
      return;
    }
      ...
      <div className="border-b border-slate-200 dark:border-white/10 pb-6">
        ...
        >
          ...
          <p className="font-display text-slate-500 dark:text-white/50 text-sm mt-2 uppercase tracking-wide">
            {isAllEnterprises ? 'Assign pickup requests to logistics partners' : `Pickups for ${selectedEnterprise?.name || 'selected enterprise'}`}
          </p>
        </motion.div>
      </div>
      ...
      >
        ...
        <div className="border border-blue-400/30 bg-blue-400/5 p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-mono text-xs text-slate-500 dark:text-white/50 uppercase">Assigned</span>
          </div>
        ...
        <div className="border border-slate-400/30 bg-slate-400/5 p-5">
          <div className="flex items-center gap-2 mb-2">
            <User className="w-4 h-4 text-slate-400" />
            <span className="font-mono text-xs text-slate-500 dark:text-white/50 uppercase">Partners</span>
          </div>
          <p className="font-brand font-bold text-3xl text-slate-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {filteredRequests.length > 0 ? (
              ...
              return (
                <motion.div
                  ...
                  onClick={() => {
                    ...
                    setShowReassignMode(false);  // Reset reassign mode when selecting new request
                    setSelectedLogisticsAdmin('');
                  }}
            ...
            <div className="border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] py-16 text-center">
              <div className="w-16 h-16 border border-slate-200 dark:border-white/10 bg-white/5 flex items-center justify-center mx-auto mb-4">
                <Truck className="w-8 h-8 text-slate-500 dark:text-white/50" />
              </div>
        ...
        >
          {selectedRequestData ? (
                ...
                {selectedRequestData.special_instructions && (
                  <div>
                    ...
                    <div className="p-4 border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02]">
                      <p className="font-display text-white">{selectedRequestData.special_instructions}</p>
                    </div>
                  </div>
                )}
            ...
            <div className="border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] py-16 text-center">
              <Truck className="w-10 h-10 text-slate-500 dark:text-white/50 mx-auto mb-4" />
