...
export function SubUserDetail() {
  ...
  const userAssets = useMemo(() => {
    if (!subUser) return [];
    return assets.filter(a => a.assigned_sub_user_id === subUser.id);
  }, [assets, subUser]);
  ...
  if (!subUser) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02]">
        ...
        <p className="font-display font-bold text-slate-500 dark:text-white/50 uppercase tracking-wide mb-1">Sub-user not found</p>
        <p className="font-mono text-xs text-slate-500 dark:text-white/50 mb-6">The sub-user you're looking for doesn't exist</p>
        <button
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
            <div className="flex items-start gap-5">
              ...
              <div>
                <span className="font-mono font-bold text-xs text-ecotribe-primary tracking-[0.3em] uppercase block mb-1">Sub-User</span>
          ...
          >
            <div className="p-6 border-b border-red-500/20">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-brand font-bold text-lg text-slate-900 dark:text-white uppercase tracking-wide">Delete Sub-User</h3>
              </div>
            </div>
            <div className="p-6">
