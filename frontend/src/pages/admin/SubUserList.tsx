...
export function SubUserList() {
  ...
  const enterpriseSubUsers = useMemo(() => {
    return subUsers.map(s => {
      ...
      const assignedAssets = assets.filter(a => a.assigned_sub_user_id === s.id).length;
      const submittedAssets = assets.filter(
        a => a.assigned_sub_user_id === s.id && ['submitted', 'remote_review', 'conditionally_accepted', 'final_accepted', 'completed'].includes(a.status)
      ).length;
  ...
  const filteredUsers = useMemo(() => {
    ...
    return result;
  }, [enterpriseSubUsers, searchQuery, statusFilter, departmentFilter]);

  // Stats
  const stats = {
    total: enterpriseSubUsers.length,
    active: enterpriseSubUsers.filter(u => u.status === 'active').length,
    pending: enterpriseSubUsers.filter(u => u.status === 'pending_invite').length,
    totalAssigned: enterpriseSubUsers.reduce((sum, u) => sum + u.assignedAssets, 0),
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="font-display font-bold uppercase tracking-wide text-slate-500 dark:text-white/50">Loading sub-users...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 border-b border-slate-200 dark:border-white/10 pb-8">
        ...
        >
          <button
            onClick={() => navigate(`${basePath}/sub-users/upload`)}
          ...
          <button
            onClick={() => navigate(`${basePath}/sub-users/invite`)}
      ...
      >
        {filteredUsers.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => navigate(`${basePath}/sub-users/${user.id}`)}
          ...
          <div className="py-16 text-center">
            ...
            <p className="font-display font-bold text-slate-900 dark:text-white uppercase tracking-wide mb-1">No sub-users found</p>
      ...
      {filteredUsers.length > 0 && (
        <p className="font-mono text-xs text-slate-500 dark:text-white/50 text-center uppercase tracking-widest">
          Showing {filteredUsers.length} of {enterpriseSubUsers.length} sub-users
        </p>
      )}
    </div>
  );
