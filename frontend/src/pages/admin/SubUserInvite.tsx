...
export function SubUserInvite() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess('Invitations Sent', `${invites.length} sub-user${invites.length > 1 ? 's' : ''} invited successfully`);
    } catch (error) {
          ...
          >
            <div className="p-5 border-b border-black/10 dark:border-white/10 flex items-center justify-between">
              <h2 className="font-display font-bold text-sm text-black dark:text-white uppercase tracking-wide">
                {invites.length > 1 ? `Invite ${index + 1}` : 'Sub-User Details'}
              </h2>
              {invites.length > 1 && (
                <button
                  type="button"
