...
export function BatchApprovals() {
  ...
  // Filter batches that require Org Admin approval (was CFO)
      ...
      >
        <div className="border border-amber-400/30 bg-amber-400/5 p-5">
          ...
          <p className="font-mono text-xs text-slate-500 dark:text-white/50 mt-1">
            ₹{(approvalBatches.filter(b => b.status === 'pending_approval').reduce((sum, b) => sum + (b.estimated_value || 0), 0) / 100000).toFixed(1)}L
          </p>
        </div>
