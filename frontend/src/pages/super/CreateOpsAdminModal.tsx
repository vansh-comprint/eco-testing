...
type CreateOpsAdminForm = z.infer<typeof createOpsAdminSchema>;

interface CreateOpsAdminModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}
...
export function CreateOpsAdminModal({ isOpen, onClose, onSuccess }: CreateOpsAdminModalProps) {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<CreateOpsAdminForm>({
    resolver: zodResolver(createOpsAdminSchema),
  });
  ...
  const onSubmit = async (data: CreateOpsAdminForm) => {
    ...
    try {
      ...
      console.error('Error creating OPS Admin:', error);
  ...
  return (
    <Modal
      ...
      description="Add a new OPS Admin user (Operations/Review & QC)"
      size="md"
    >
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="space-y-4">
          ...
          <div className="pt-2">
            <p className="font-mono text-xs text-slate-600 dark:text-zinc-400">
              Role: <span className="font-bold text-slate-800 dark:text-zinc-200">OPS Admin (Operations/Review & QC)</span>
            </p>
            <p className="font-mono text-xs text-slate-500 dark:text-zinc-500 mt-1">
              This user will have access to OPS and Review & QC portals.
            </p>
          </div>
        </div>
        ...
        <ModalFooter className="mt-6">
          ...
          >
            {isSubmitting ? 'Creating...' : 'Create OPS Admin'}
          </Button>
        </ModalFooter>
      </form>
    </Modal>
