...
interface CSVUploadProps {
  enterpriseId: string;
  batchId?: string;
  onUpload: (assets: CreateAssetInput[], metadata: BulkUploadMetadata) => Promise<void>;
  onCancel?: () => void;
  isLoading?: boolean;
}
...
export function CSVUpload({ enterpriseId, batchId, onUpload, onCancel, isLoading }: CSVUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await onUpload(assets, metadata);
...
4. Pre-assigning users will automatically create sub-user accounts and send invitations
...
`;
  ...
  const downloadErrorsSheet = async () => {
    ...
    // Define columns - same as template plus Errors column
    worksheet.columns = [
        ...
        >
          ...
          <div className="bg-white/95 dark:bg-black/40 backdrop-blur-md border border-black/10 dark:border-white/10 btn-chamfer">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 border-l border-t border-white/10">
              ...
              <div className={`p-5 border-r border-b border-white/10 ${uniqueUsers.size > 0 ? 'bg-blue-500/5' : ''}`}>
                <div className="flex items-center justify-between mb-2">
              ...
              <div className={`p-5 border-r border-b border-white/10 ${rowsWithUsers.length > 0 ? 'bg-blue-500/5' : ''}`}>
                <div className="flex items-center justify-between mb-2">
          ...
          {uniqueUsers.size > 0 && (
            <div className="border border-blue-400/20 bg-blue-400/5 p-5">
              <div className="flex gap-4">
                ...
                <div>
                  <p className="font-display font-bold text-sm text-white uppercase tracking-wide mb-2">
                    {uniqueUsers.size} Sub-User{uniqueUsers.size > 1 ? 's' : ''} Will Be Invited
                  </p>
                  <div className="flex flex-wrap gap-2">
