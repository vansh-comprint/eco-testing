...
| OR-07 | Ops Branch > Batches | Batch status not updating after payouts are processed (same issue on IT Admin side) | State/Logic | High | OPEN |
...
| SA-02 | QC | Filters only work client-side — need server-side filtering | Filter/Backend | High | OPEN |
